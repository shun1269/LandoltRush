...
## 現行ルール（サイズ・回転速度、日本語UI・ライフ制への改訂）

- 成功時は取得した環の形状・位置・サイズ・切れ目の向きを引き継いだ表示用コピーを作る。
- 約0.6秒で画面中央へ移動しながら加速して拡大し、内側の縁が画面の四隅より外へ抜ける。黒から青緑へ変わり、拡大に合わせてフェードする。
- 連続取得の成功演出は重ねて再生する。最大16個の表示を再利用し、当たり判定・スコア・生成上限には影響させない。再挑戦・タイトルに戻る操作で消去する。

- 環のスケールは1.2〜1.6。最小直径は参考画像の右側の環より20％大きくする（1600×900で約216px）。
- 回転速度の基準範囲は180〜240度/秒。移動経路と速度、外側半径から環全体が画面内に収まる時間を求め、その間に最低2回転する速度を下限にする。移動速度を維持するため、短い経路では必要な場合だけ設定上限より速くする。最低回転数はSpawnParam.MinVisibleRotationsで管理し、0で補正なしにできる。
- 成功・ミスによる途中消滅を除く。画面に収まらない特殊な設定では中心が画面内にある時間を使う。回転方向は引き続きランダム。
- 移動速度は1.5〜3を維持する。

- タイトル「Landolt Rush」以外のUIは日本語にする。タイトル画面はタイトルと開始・終了操作に絞る。
- 最大ライフは3。シンプルなハートで残り数を表す。
- 先端・胴体の接触と画面外への見逃しをすべてミスとし、環1個につきライフを1つ減らす。先端接触でも即終了しない。
- ミスでは「ミス」のみを表示し、画面の縁を赤くして約0.55秒で消す。コンボはリセットし、スコアは維持する。
- ライフ0で全ての環と操作を停止。最後のダメージ演出を見せた後、約0.6秒で結果を表示する。
- リザルトの記録項目はスコアと最大コンボのみ。再挑戦・タイトルへの操作を用意する。
- コンボは画面中央の背景に大きな数字で表示し、その下に継続ゲージを置く。増加した瞬間に数字を1.3倍へ拡大し、色も少し濃くして約0.24秒で元に戻す。連続増加では拡大を再発火し、ミスやタイトル移行で即リセットする。
- タイトル画面の環は時計回りに毎秒30度の一定速度で回転する。往復や速度の揺らぎは付けない。
- 複数の環の同時ミスでも1個につき1ダメージ。終了後はそれ以上ダメージや得点を加算しない。
- 内側半径への進入、コンボ6秒、複数同時出現と4秒→0.9秒の加速は維持する。

## 前回の改訂記録（上記と異なる部分は更新済み）

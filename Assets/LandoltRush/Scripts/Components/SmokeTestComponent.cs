...
namespace LandoltRush
{
    ...
    public sealed class SmokeTestComponent : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            Vector2 direction=new Vector2(10,-6.5f).normalized;
            rod.TestPosition=direction*1.3f;Scope.Input.StartButton.onClick.Invoke();yield return null;
            presenter.Spawn(StaticRing());rod.TestPosition=direction*.45f;yield return null;yield return null;
            if(!Require(data.SuccessCount==1&&data.Score==100,"first success",directory))yield break;
            rod.TestPosition=direction*1.3f;rod.Sample(Scope.RodParam);presenter.Spawn(StaticRing());rod.TestPosition=direction*.45f;yield return null;yield return null;
            ...
            // Restart isolates the next case from the independently scheduled spawns.
        ...
        static RingSpawnData StaticRing()=>new RingSpawnData{Position=Vector2.zero,Scale=1,Angle=Mathf.Atan2(-6.5f,10)*Mathf.Rad2Deg};

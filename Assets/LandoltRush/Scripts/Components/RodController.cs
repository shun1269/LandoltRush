...
namespace LandoltRush
{
    public sealed class RodController : MonoBehaviour
    {
        ...
        public Vector2 PreviousPivot {get;private set;}
        ...
        {CanMove=active;View.gameObject.SetActive(active);if(active){Read(param);PreviousTip=Tip;PreviousPivot=Pivot;}}
        public void Sample(RodParam param) {PreviousTip=Tip;PreviousPivot=Pivot;if(CanMove)Read(param);}
        void Read(RodParam param)
        {
            ...
            Pivot=new Vector2(Bounds.xMax+param.RightPivotMargin,Bounds.yMin-param.BottomPivotMargin);
            View.Shaft.widthMultiplier=param.ShaftRadius*2;
            View.Render(Pivot,Tip,param.TipRadius);
        }

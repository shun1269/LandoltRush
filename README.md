...
環のサイズは **1.2〜1.6倍**。最小サイズは、調整前の参考画像の右側の環より直径20％大きく、1600×900表示では約216pxです。回転速度の基準範囲は **180〜240度/秒**。最低値を維持し、上限を360から240度/秒へ下げて全体を遅めに調整しています。経路・移動速度・半径から計算した「環全体が画面内に収まる時間」で **最低3回転** するよう引き上げます（成功・ミスで途中消滅する場合を除く）。必要な場合は設定上限240度/秒も超えます。移動速度は1.5〜3のままです。設定は `SpawnParam` にあります。

...
環のサイズは **1.2〜1.6倍**。最小サイズは、調整前の参考画像の右側の環より直径20％大きく、1600×900表示では約216pxです。回転速度の基準範囲は **180〜240度/秒**。経路・移動速度・半径から計算した「環全体が画面内に収まる時間」で **最低2回転** するよう引き上げます（成功・ミスで途中消滅する場合を除く）。移動速度1.5〜3を維持するため、短い経路では必要な場合だけ設定上限240度/秒を超えます。設定は `SpawnParam` にあり、`MinVisibleRotations` で最低回転数を調整できます（0で補正なし）。
